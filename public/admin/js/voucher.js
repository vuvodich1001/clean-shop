        ...
        .then(posts => {
            ...
            if (posts.length != 0) {
                ...
                const categories = posts.map(voucher => {
                    ...
                    <td><a href="" class="btn-delete-voucher" voucher-id="${voucher.discount_id}"><i class="fas fa-trash-alt"></i></a>
                        <a href="" class="btn-update-voucher" voucher-id="${voucher.discount_id}"><i class="fas fa-edit"></i></a>
                    </td>
                </tr>
                    `;
                })
...
function generateRandomString(length = 8) {
    let characters = 'abcdefghijklmnopqrstuvwxyzaskdhfhfABCDEFGHIJKLMNksadfOPQRSTUVWXYZ';
...
async function getAllVoucher() {
    const categories = await fetch('index.php?controller=Voucher&action=getAllVoucher')
        .then(response => response.json())
        .then(posts => {
            return posts;
        });
